  ...
  return (
    <Card className="gap-3">
      ...
      <CardContent className="grid items-center gap-3 sm:grid-cols-[minmax(180px,1fr)_minmax(150px,0.8fr)]">
        <div className="relative h-[230px] min-w-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={data} dataKey="value" nameKey="name" innerRadius="58%" outerRadius="86%" stroke="white" strokeWidth={2}>
          ...
          <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
            <strong className="text-2xl font-bold">{formatNumber(total)}</strong>
        ...
        <div className="flex flex-col gap-4">
          {data.map((item) => (
            <div key={item.name} className="flex items-start gap-2.5">
              <span className="mt-1 size-3 shrink-0 rounded" style={{ backgroundColor: item.color }} />
              <div>
      ...
      <CardFooter className="justify-center">
        <Link href="/reports" className="text-sm font-semibold text-[#0a43b8] hover:underline">View full report</Link>
      </CardFooter>
    </Card>
  );
}
